...
# Idea Lab — objective topic strategy
...
```text
Perplexity → IndustryResearchOpportunity[] → subjects/seeds
  → objectiveTopicStrategies → shared scoring / support keys → TopicCandidateGenerationResult
```
...
| Layer | Owns | Must not |
|-------|------|----------|
| `subjects/` | Classify nouns (`classifyContextSubjects` orchestrator + extractors) | Frame titles / Hooked copy / score |
| `gtc/frame-title` | Educational frame only | Hooked itch |
| `gtc/topic-title-hook` | Candidate title Trigger | Change kind / evidence / support keys |
...
Support keys stay seed/subject-based — title framing and hooks must not unlock completeness. Default title hooks are deterministic templates. `TOPIC_TITLE_HOOK_PROVIDER=openai` is **remapped to deterministic** (OpenAI title-hook path is not live). Optional post-rank title polish uses `TOPIC_TITLE_POLISH_PROVIDER=openai`. Direction punchline polish is separate (`HOOK_ENRICHMENT_PROVIDER=openai`).
