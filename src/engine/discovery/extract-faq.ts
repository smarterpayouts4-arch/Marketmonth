...
function pushUnique(
  ...
  if (q.length < 8 || a.length < 4) return;
  if (!/\?/.test(q) && !/^(what|why|how|when|where|who|do|does|is|are|can)/i.test(q)) {
    return;
  }
...
function fromJsonLd(html: string, sourceUrl: string, out: FaqEntry[], seen: Set<string>) {
...
function fromAccordion(html: string, sourceUrl: string, out: FaqEntry[], seen: Set<string>) {
  const $ = cheerio.load(html);
  $("details").each((_, el) => {
  ...
  $("[itemtype*='Question'], .faq-item, .faq__item, li.faq").each((_, el) => {
    const q =
      $(el).find("[itemprop='name'], .faq-question, .question, h3, h4").first().text() ||
      $(el).find("summary, button").first().text();
    const a =
