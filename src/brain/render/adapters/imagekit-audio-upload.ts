  ...
  try {
    ...
    if (!res.ok) {
      throw new ImageBytesError(
        ...
        `Audio storage upload failed (${res.status})`,
        res.status === 429
      );
    }
    ...
    if (!data.fileId || !data.url) {
      throw new ImageBytesError(
        "image_storage_upload_failed",
        "Audio storage returned incomplete upload result",
        true
      );
    }
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new ImageBytesError(
        ...
        "Audio storage upload timed out",
        true
      );
    }
    throw new ImageBytesError(
...
/**
 * Best-effort delete of a previously uploaded voice file.
 * Failures are swallowed by callers — clear must still omit bundle voice.
 */
export async function deleteSceneVoiceFromImageKit(
  storageFileId: string
...
/** Upload scene voice bytes to ImageKit under the Short scene folder. */
export async function uploadSceneVoiceToImageKit(input: {
  atomId: string;
  sceneId: string;
  bytes: Buffer;
  mimeType: string;
  ...
  const folder = `marketmonth/atoms/${atomId}/youtube-short/scenes/${sceneId}/voice`;
