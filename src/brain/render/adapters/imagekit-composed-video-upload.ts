  ...
  try {
    ...
    if (!res.ok) {
      throw new ImageBytesError(
        ...
        `Composed video storage upload failed (${res.status})`,
        res.status === 429
      );
    }
    ...
    if (!data.fileId || !data.url) {
      throw new ImageBytesError(
        "image_storage_upload_failed",
        "Composed video storage returned incomplete upload result",
        true
      );
    }
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new ImageBytesError(
        ...
        "Composed video storage upload timed out",
        true
      );
    }
    throw new ImageBytesError(
      "image_storage_upload_failed",
      "Composed video storage upload failed",
      true
    );
  } finally {
    clearTimeout(timer);
...
/** Upload composed scene MP4 under a dedicated folder (not Veo /video). */
export async function uploadSceneComposedVideoToImageKit(input: {
  atomId: string;
  sceneId: string;
  bytes: Buffer;
  mimeType: string;
  ...
  const folder = `marketmonth/atoms/${atomId}/youtube-short/scenes/${sceneId}/composed`;
