...
| Channel package | Channel specialists + Studio Video adapter | Shared envelope `src/brain/channels/package-envelope.schema.ts`; YouTube Short `youtube-short/` (`youtube-short-service`, duration-policy, draft); formats in `src/brain/content-studio/` | Other channels `not_connected` scaffolds | `produce-content-bundle` → Short service + Video adapter | Fake generation on `not_connected` channels; resurrecting deleted `youtube-short-adapter.ts` | **channelRegistry:** only YouTube Short `enabled`; Video via format registry; Studio tabs read `PLATFORM_REGISTRY` only (ADR 0006) | Maturity |
| Content Studio format package | Content Studio (transitional orchestrator) | `src/brain/content-studio/` (`platform-registry.ts`, `schemas/format-package.ts`, Video adapter only) | Orchestration `produce-content-bundle.ts`; Short adapter **deleted** (Phase 2.1) | `/content?atomId=` Studio, production API | Confusing Studio Video with `youtubeLong` channel; deleting content-studio before Video owner | YouTube Short + Video formats **Live**; Short runtime = `youtube-short-service`; ADR 0006 | Immediate regression |
| Production bundle store | Content Studio persistence | `src/brain/content-studio/bundle-store.ts` → `data/runtime/production-bundles/` | Not Neon; schema in `format-package.ts`; durable edits = in-package fields (`youtubeShortDurableEditsSchema`) | Production GET/POST, atom Studio shell | Inventing a second Short edit store; treating JSON as multi-instance SoT | Idempotent JSON bundles keyed by atom; **dev/single-instance**; Neon before scale; export/render stubs | Maturity |
